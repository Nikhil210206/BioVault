...
const Unlock = () => {
  ...
  const handleUnlock = async () => {
    if (!method || !username) {
        addToast("error", "Please select a method and be logged in.");
        return;
    }
    ...
    try {
      // Use the stored username for the unlock request
      const response = await unlockVault({
        username: username,
        method,
        proof: "demo-proof-data", // This is still a placeholder
      });
        ...
        >
          <motion.div variants={shouldReduce ? {} : fadeInUp} className="text-center mb-12">
            ...
            <p className="text-lg text-muted-foreground">
              Welcome, {username || 'guest'}! Choose your authentication method.
            </p>
          </motion.div>
              ...
              {method && unlockStatus === "idle" && !isUnlocking && (
                ...
                >
                  ...
                  <p className="text-muted-foreground mb-8">
                    Click the button to unlock your vault.
                  </p>
                  <motion.button
                    {...(shouldReduce ? {} : buttonTap)}
